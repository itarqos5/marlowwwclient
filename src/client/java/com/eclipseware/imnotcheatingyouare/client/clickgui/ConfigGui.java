...
public class ConfigGui extends Screen {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        // Background
        guiGraphics.fill(0, 0, w, h, new Color(10, 10, 10, 200).getRGB());
        guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Cloud Config Manager", w / 2, 20, new Color(230, 10, 230).getRGB());
        guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Select modules to include in your export", w / 2, 35, new Color(170, 170, 170).getRGB());

        // Buttons
        guiGraphics.fill(w / 2 - 160, h - 40, w / 2 - 10, h - 20, new Color(30, 30, 30).getRGB());
        guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Export to Clipboard", w / 2 - 85, h - 34, -1);

        guiGraphics.fill(w / 2 + 10, h - 40, w / 2 + 160, h - 20, new Color(30, 30, 30).getRGB());
        guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Import from Clipboard", w / 2 + 85, h - 34, new Color(230, 10, 230).getRGB());
...
guiGraphics.pose().translate(0f, (float) scrollY);
    ...
    int y = 60;
        for (Module m : ImnotcheatingyouareClient.INSTANCE.moduleManager.modules) {
            ...
            guiGraphics.fill(w / 2 - 80, y, w / 2 - 65, y + 15, included ? new Color(230, 10, 230).getRGB() : new Color(40, 40, 40).getRGB());
            guiGraphics.drawString(Minecraft.getInstance().font, m.getName(), w / 2 - 55, y + 4, included ? -1 : new Color(100, 100, 100).getRGB(), false);
            y += 20;
        }
