        ...
        if (selectedModule != null) {
            ...
            if (comps.isEmpty()) {
                guiGraphics.drawString(Minecraft.getInstance().font, "Nothing to config here!", (int)posX + 130, (int)posY + 65, new Color(170, 170, 170).getRGB(), false);
                                ...
                                // Apply scissor clipping and scroll matrix
                guiGraphics.enableScissor((int)posX + 120, (int)posY + 55, (int)(posX + windowWidth), (int)(posY + windowHeight));
                guiGraphics.pose().pushMatrix();
...
int button = event.button();
    ...
    Module arrayListMod = ImnotcheatingyouareClient.INSTANCE.moduleManager.getModule("ArrayList");
        ...
        // Only pass clicks to settings if inside the right panel
        if (isInside(mouseX, mouseY, posX + 120, posY + 55, posX + windowWidth, posY + windowHeight)) {
...
// Providing alternative signatures to intercept scroll regardless of the mapping changes
...
private boolean handleScroll(double mouseX, double mouseY, double scrollY) {
    if (isInside(mouseX, mouseY, posX + 120, posY + 55, posX + windowWidth, posY + windowHeight)) {
        ...
        if (scrollOffset > 0) scrollOffset = 0; // Prevent scrolling down past the top
        ...
        // Allow scrolling if the content exceeds the visible area
        double maxScroll = -((maxH + 35) - windowHeight);
    ...
    public void loadComponents(Module m) {
        comps.clear();
        scrollOffset = 0; // Reset scroll back to the top when swapping modules
        int sY = 65; 
        ...
        if (ImnotcheatingyouareClient.INSTANCE.settingsManager.getSettingsByMod(m) != null) {
            ...
            for (Setting setting : ImnotcheatingyouareClient.INSTANCE.settingsManager.getSettingsByMod(m)) {
                ...
                // AutoShieldBreaker & BreachSwap: Dynamic Swap Back settings
